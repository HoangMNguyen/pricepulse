...
| SQLAlchemy | 2.0 Core, engine lifecycle in Lambda (NullPool, cold-start secret lookup, resume retry) | [`db/engine.py`](../src/pricepulse/db/engine.py) |
...
| Secrets & least privilege | SSM SecureString + KMS per function, least-privilege DB roles on a managed provider, cost/latency-driven database migration with measured evidence | [`database.tf`](../infra/envs/dev/database.tf), [`scripts/bootstrap_db.sh`](../scripts/bootstrap_db.sh), [ADR-0009](adr/0009-neon-instead-of-aurora.md) |
